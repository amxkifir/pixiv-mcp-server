...
# 在所有其他操作之前加载 .env 文件，以确保配置在初始化时可用
load_dotenv()
...
async def auth(token: str) -> str:
    ...
    try:
        await asyncio.to_thread(state.api.auth, refresh_token=token)
        state.is_authenticated = True
        state.user_id = state.api.user_id
        state.refresh_token = token
        return f"认证成功！用户ID: {state.user_id}"
    except Exception as e:
...
async def search_autocomplete(word: str) -> str:
    """获取与关键词相关的热门搜索标签。"""
    json_result = await asyncio.to_thread(state.api.search_autocomplete, word)
    error = handle_api_error(json_result)
    if error:
        return error
